...
app.get('/team_stats', function(req, res) {
	...
	var losses = 'SELECT COUNT(home_score < visitor_score) FROM football_games;';
	db.task('get-everything', task => {
        return task.batch([
...
app.get('/player_info', function(req, res) {
	var footballPlayerInfo = 'SELECT name,id FROM football_players;';
	db.task('get-everything', task => {
        return task.batch([
            task.any(footballPlayerInfo),	
        ]);
    })
    .then(info => {
...
app.get('/player_info/post', function(req, res) {
	var footballPlayerInfo = 'SELECT name,id FROM football_players;';
	var totalGames = 'SELECT COUNT(*) FROM (SELECT id FROM football_players, football_games ' + 'WHERE football_players.id = ANY(football_games.players)) ' + 'AS subq WHERE subq.id = ' + req.query.player_choice + ';';
	var specificInfo = 'SELECT * FROM football_players WHERE football_players.id = ' + req.query.player_choice + ';';
	db.task('get-everything', task => {
        return task.batch([
            task.any(footballPlayerInfo),
            task.any(totalGames),
            task.any(specificInfo)
        ]);
    })
    .then(info => {
    	res.render('pages/player_info',{
				...
				data: info[0],
                                gamesPlayed: info[1][0],
                                playerInformation: info[2][0]
